...
export default withApiAuthRequired(async function getStats(
    req: NextApiRequest,
    res: NextApiResponse
) {
    try {
        ...
        const result = await Promise.all(
            schoolYears.map(async (year) => {
                const yearMajors = await bachelorStudent.aggregate([
                    { $match: { school_year: year } },
                    {
                        $group: {
                            ...
                            paidCount: {
                                $sum: {
                                    $cond: [{ $eq: ["$paid_ksk", "Да"] }, 1, 0],
                                },
                            },
                            acceptedCount: {
                                $sum: {
                                    $cond: [
                                        { $eq: ["$status_of_ksk", "Приет"] },
                                        1,
                                        0,
                                    ],
                                },
                ...
                return {
                    schoolYear: year,
                    majors: yearMajors.map(
                        ({ _id, askCount, paidCount, acceptedCount }) => ({
                            major: _id,
                            askCount,
                            paidCount,
                            acceptedCount,
